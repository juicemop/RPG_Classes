...
public class IceWandProjectile extends Projectile {
    public IceWandProjectile() {
    }

    public IceWandProjectile(Level level, Mob owner, float x, float y, float targetX, float targetY, float speed, int distance, int knockback) {
        this.setLevel(level);
        this.setOwner(owner);
        this.x = x;
        this.y = y;
    ...
    public Trail getTrail() {
        return new Trail(this, this.getLevel(), new Color(0, 128, 255), 4.0F, 500, this.getHeight());
    }
    ...
    public void doHitLogic(Mob mob, LevelObjectHit object, float x, float y) {
        ...
        getLevel().entityManager.addLevelEvent(new IceWandExplosionLevelEvent(x, y, 50, 0, getOwner()));
    }
}