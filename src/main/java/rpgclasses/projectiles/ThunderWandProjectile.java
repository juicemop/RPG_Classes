...
public class ThunderWandProjectile extends Projectile {
    public ThunderWandProjectile() {
    }

    public ThunderWandProjectile(Level level, Mob owner, float x, float y, float targetX, float targetY, float speed, int distance, int knockback) {
        this.setLevel(level);
        this.setOwner(owner);
        this.x = x;
        this.y = y;
    ...
    public Trail getTrail() {
        return new Trail(this, this.getLevel(), new Color(255, 255, 0), 4.0F, 500, this.getHeight());
    }
    ...
    public void doHitLogic(Mob mob, LevelObjectHit object, float x, float y) {
        ...
        getLevel().entityManager.addLevelEvent(new ThunderWandLevelEvent(getOwner(), (int) x, (int) y));
    }
}
